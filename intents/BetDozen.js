...
module.exports = {
  handleIntent: function() {
    ...
    if (!this.event.request.intent.slots.Ordinal
      ...
      // Sorry - reject this
      speechError = res.strings.BETDOZEN_INVALID_DOZEN;
      reprompt = res.strings.BET_INVALID_REPROMPT;
    } else {
      dozen = res.valueFromOrdinal(this.event.request.intent.slots.Ordinal.value);
      if (!dozen) {
        speechError = res.strings.BETDOZEN_INVALID_DOZEN_VALUE.replace('{0}', this.event.request.intent.slots.Ordinal.value);
        reprompt = res.strings.BET_INVALID_REPROMPT;
      } else {
        const bet = {};
        ...
        if (isNaN(bet.amount) || (bet.amount == 0)) {
          ...
          ssml = res.strings.BETDOZEN_PLACED.replace('{0}', bet.amount).replace('{1}', dozen).replace('{2}', reprompt);
        }
      }
    }
